...
stimulation_parameters = struct( ...
    ...
    'reps_per_set',             C.THRESH_REPS_PER_SET,...
...
recording_parameters = struct( ...
    'baseline_duration',1,...   %round(sum(reps_per_set)/frequency*1.25),...
    'analog',           [],...
    'digital',          [],...
    'spikes',           []);
...
recording_parameters.analog  =  struct( ...
    ...
    'channels',         {C.TRIPOLAR.CUFF_CHANNELS{:}},... % mat2cell(reshape(channels,[2 nCuffs])',ones(nCuffs,1),2), ...
    'upsample',         C.TRIPOLAR.UPSAMPLE, ...
    'filter_pipeline',  {C.TRIPOLAR.FILTER_PIPELINE}, ...
    'invert',           false,...           %C.TRIPOLAR.INVERT_CHANNEL(1:nCuffs)', ...
    'labels',           C.TRIPOLAR.CUFF_LABELS);          % mat2cell(reshape(labels,[2 nCuffs])',ones(nCuffs,1),2));
...
analysis_parameters = struct( ...
    ...
    'single_rms_threshold',     false,                      ...
    'rms_threshold_multiplier', C.RMS_THRESHOLD_MULTIPLIER);


experiment = ThresholdSearchExperimentProtocol( ...
    'catName',                  C.CAT_NAME,...
...
experiment.bPromptUser = prompt_user;
run(experiment)