function C = experiment_constants_Errol
% =========================================================================
% ANIMAL AND HEADSTAGE BACKGROUND
% =========================================================================
C.CAT_NAME            = 'Errol';   % Animal name
...
C.THRESH_REPS         = 300;            % number of pulses applied in full data collection trials
...
%-------------------------------------------------------------------------------------------------
% C.STIM_MAP            = num2cell([1:20; 5:24]'); % BIPOLAR VERTICAL
%-------------------------------------------------------------------------------------------------
% a = [1:16]';           % TRIPOLAR (VERTICAL)
...
C.SEARCH_CUFFS_IDX = [1:6 8]; %which cuffs to include in the binary search (in case you want to visualize others but not search them)
...
monopolar_cuff_mapping    = { ...
%     [10 11]  'Sciatic Proximal' 
     % [15]  'EUS EMG (part grounded)' 
    };
...
% C.MONOPOLAR.CUFF_CHANNELS = monopolar_cuff_mapping(:,1)';
% C.MONOPOLAR.CUFF_LABELS     = monopolar_cuff_mapping(:,2)';
