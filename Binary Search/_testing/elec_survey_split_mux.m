function chanSnips = elec_survey_split_mux(C, filepath, varargin)
...
if max(numEvts) == min(numEvts) && isempty(C.HIGHAMP_HEADSTAGE_LOC) % no overlapping events, only one headstage
 ...
 for n = 1:length(numEvts)
     evtsPassed(n) = sum(numEvts(1:n));
 end

 evtsPassed = [0 evtsPassed];
...
for j = 1:numChans
    ...
    if stim_blanking
        ...
        for k=1:size(stimIdx, 1) %for all channels
            for n=1:size(stimIdx, 2) %for all events
                ...
                temp = interp1([stimIdx(k, n)-1 (stimIdx(k, n)+1+blankTime)], ... %ERROR IS HERE - k = 3, n = 251
    ...
    for i = 1:sum(numEvts)/C.MAX_AMP_REPS
        %get segment just before and for a bit after stimulation on each
        %channel set
        endval1 = C.MAX_AMP_REPS*i;
        rowsplit = find(endval1>evtsPassed, 1, 'last');
        
        endval = C.MAX_AMP_REPS*i-evtsPassed(rowsplit);
        startval = endval-C.MAX_AMP_REPS+1;
        
        %tmpIdx = ceil(median(diff(stimIdx(i, :))));
        %put results in an array that is chanSnips{nervecuffnum, stimchannum}
        %TODO this part defs won't work with MUX - need to input which
        %channels were stimulated when so I can manually set it.
        chanSnips{j, i} = cell2mat(arrayfun(@(x) tmpChan((round(x-pre_window)):(round(x+window_len))), stimIdx(rowsplit, startval:endval),'UniformOutput',false))';
    end
end
