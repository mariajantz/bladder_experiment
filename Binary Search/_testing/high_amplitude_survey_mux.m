function [stim_freqs, response_locs] = high_amplitude_survey_mux(C, savepath, datapath, baseline_filenum, varargin)
...
elseif ~isnan(survey_filenum)
    ...
    pause(1);
    stimOrder = elec_survey_stim_mux(C, 0.2);
else
    ...
    stimOrder = elec_survey_stim_mux(C, 0.2, sprintf('%s\\datafile', datapath));
