...
saveloc = 'E:\2020\Neville\CystometryPlots'; 
save_iter = 1; 
save_on = 1; 
a = arduino; 
figure(2);
set(gcf, 'Position', [8 42 699 1026])
set(gcf,'color','w');
...
scrollh = animatedline('MaximumNumPoints', scroll_plot_length*30, 'Linewidth', 2, 'Color', clr(1, :));
...
while t>0
    ...
    chan1(t) = readVoltage(a, 'A0')/50*1000; 
...
%     title('Last 2 minutes'); 
    ...
    elseif x(t) >= static_plot_length && x(t) < static_plot_length*2
        ...
        %save plot
        if mod(save_iter, 2)==0 && save_on
            savefig(fullfile(saveloc, sprintf('pressure_%04d', save_iter))); 
        end
